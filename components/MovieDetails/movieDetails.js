  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("additionnalDataVideo", additionnalDataVideo.data.results[0].key);
    ...
    <>
      {moviesData.length > 0 && (
        ...
        >
          ...
          <iframe width="560" height="315" src={`https://www.youtube.com/embed/${moviesData[0].video}`} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
        </div>
      )}
    </>
  );
